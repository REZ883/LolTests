...
if not is_admin():
    ...
    try:
        ...
        # Exécute le script avec ShellExecuteW pour demander l'élévation via UAC
        ctypes.windll.shell32.ShellExecuteW(None, "runas", sys.executable, params, None, 1)
        print("Demande d'élévation des privilèges administrateur...")
        sys.exit(0) # Quitte le script non élevé
    except Exception as e:
        print(f"Erreur lors de la tentative d'élévation des privilèges: {e}")
        sys.exit(1) # Quitte si l'élévation échoue
else:
    print("Le script s'exécute avec les privilèges administrateur.")
from PySide6.QtWidgets import (
    QApplication, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout, QPushButton,
    QLabel, QScrollArea, QFrame, QProgressBar, QGridLayout, QLineEdit,
    QDialog, QMessageBox, QDialogButtonBox, QSpacerItem, QSizePolicy, QStyle
)
...
def get_latest_version():
    try:
        ...
        print(f"Err version DDragon: {e}")
        return None

def download_json(url, save_path):
    try:
        print(f"DDragon DL: {url}")
...
def load_json(path):
    try:
        ...
        print(f"Échec load JSON {path}: {e}")
        return None
...
def sauvegarder_comptes(comptes):
    ...
    try:
        ...
        print(f"Err sauvegarde comptes:\n{e}")
...
def charger_comptes():
    ...
    try:
        ...
        if not isinstance(comptes, dict): raise json.JSONDecodeError("Format invalide", "", 0)
        comptes_copy = comptes.copy()
        for account_key, info in comptes_copy.items():
            if not isinstance(info, dict):
                 print(f"Format entrée invalide pour {account_key}, suppression.")
                 del comptes[account_key]
                 needs_save = True
                 continue

            if 'region' not in info: info['region'] = "EUW1"; needs_save = True
            ...
            if 'password' not in info or not isinstance(info.get('password'), str):
                print(f"Mot de passe manquant/invalide pour {account_key}, réinitialisation.")
        ...
        print(f"Fichier '{json_path}' absent, création défaut.")
        sauvegarder_comptes(default)
        return default.copy()
    except json.JSONDecodeError as e:
         print(f"Erreur JSON dans '{json_path}': {e}. Création défaut.")
         sauvegarder_comptes(default)
         return default.copy()
    except Exception as e:
        print(f"Err chargement comptes: {e}")
        return default.copy()
    ...
    if hwnd:
        try:
            ...
            try:
                ...
                print(f"SetForegroundWindow échoué ({e_fg}), tentative SW_SHOW")
                win32gui.ShowWindow(hwnd, win32con.SW_SHOW)
            time.sleep(0.1)
            if win32gui.GetForegroundWindow() == hwnd:
                print("Fenêtre Riot activée.")
                return hwnd
            else:
                print("Avert: Fenêtre Riot trouvée mais pas au premier plan.")
                return hwnd
        except Exception as e:
            print(f"Err win32 activation: {e}")
            return None
    else:
        return None
...
def attendre_processus(process_name_part, timeout=20):
    start = time.time()
    print(f"Attente processus contenant '{process_name_part}' (max {timeout}s)...")
    proc_name_lower = process_name_part.lower()
    while time.time() - start < timeout:
        for proc in psutil.process_iter(['name']):
            try:
                if proc_name_lower in proc.info['name'].lower():
                    print(f"Processus '{proc.info['name']}' trouvé.")
                    return True
            except (psutil.NoSuchProcess, psutil.AccessDenied, psutil.ZombieProcess):
                continue
        time.sleep(1)
    print(f"Timeout: Processus '{process_name_part}' non trouvé.")
    return False
    ...
    def run(self):
        ...
        for key, info in self.comptes_to_fetch.items():
            ...
            if cached_entry and (current_time - cached_entry.get("timestamp", 0)) < CACHE_TTL_LIST:
                 print(f"Cache hit for summoner_info: {key}")
        ...
        with self.parent_app.api_executor as executor:
            for key, info in accounts_to_fetch:
                ...
                if gn and tl:
                    future = executor.submit(self.parent_app.get_summoner_info_sync, gn, tl, rg)
                    tasks[future] = key
                else:
            ...
            for future in as_completed(tasks):
                ...
                try:
                    ...
                    print(f"Error fetching summoner info for {key}: {exc}")
...
class DetailFetchThread(QThread):
    ...
    def run(self):
        ...
        print(f"Thr-Detail: Fetch Détails {self.account_key} (PUUID: {self.puuid[:8]}..., Region: {self.region})")
        mastery_data = None
        match_stats = None
        current_time = time.time()
        ...
        if cached_mastery and (current_time - cached_mastery.get("timestamp", 0)) < CACHE_TTL_DETAIL:
            print(f"Cache hit for mastery: {self.account_key}")
        ...
        if cached_stats and (current_time - cached_stats.get("timestamp", 0)) < CACHE_TTL_DETAIL:
            print(f"Cache hit for match_stats: {self.account_key}")
        ...
        if needs_mastery_fetch or needs_stats_fetch:
            try:
                ...
                print(f"Erreur dans DetailFetchThread pour {self.account_key}: {e}")


        if self.parent_app.app_running:
            print(f"Thr-Detail: Mise en file détails {self.account_key}")
    ...
    def run(self):
        try:
            if est_client_riot_actif():
                ...
                print("Client Riot/LoL détecté, tentative de fermeture...")
                if not self.parent_app.fermer_client_lol_sync():
                    print("Avert: Échec de la fermeture complète du client existant.")


            print(f"Lancement et connexion pour {self.username}...")
            login_success = self.parent_app.lancer_client_lol_sync(self.username, self.password)

            if login_success:
                 print(f"Connexion réussie (ou au moins tentée) pour {self.username}.")

            else:
                print(f"Échec lancement/connexion pour {self.username}.")


        except Exception as e:
              print(f"Erreur majeure dans le thread de connexion: {e}")
              import traceback
              traceback.print_exc()
...
class LolSwitcherApp(QMainWindow):
    ...
    def __init__(self):
        ...
        self.api_key = "RGAPI-649fd296-fcf4-4f26-904d-3fda29c61fa9"
        ...
        # Start with a synchronous refresh for immediate display
        self._refresh_ui_list_sync()
        # Optionally trigger a background check shortly after if desired
        # QTimer.singleShot(5000, self.start_update_thread)
        ...
        if os.path.exists(API_CACHE_FILE):
            try:
                with open(API_CACHE_FILE, 'r', encoding='utf-8') as f:
                    print("Chargement du cache API...")
                    return json.load(f)
            except Exception as e:
                print(f"Erreur chargement cache API: {e}")
        return {}
    ...
    def _save_api_cache(self):
        try:
            ...
            print("Cache API sauvegardé.")
        except Exception as e:
            print(f"Erreur sauvegarde cache API: {e}")
    ...
    def _setup_ddragon_worker(self):
        print("Config DDragon...")
        self.ddragon_version = get_latest_version()
        if not self.ddragon_version:
            print("Err version DDragon.")
            return
        print(f"DDragon Ver: {self.ddragon_version}")
        ...
        if not champ_data:
            ...
            if download_json(url, champ_path):
                ...
                print(f"Échec DL champ v{self.ddragon_version}.")
                return
        ...
        if champ_data and 'data' in champ_data:
            try:
                ...
                print(f"DDragon: {len(self.champion_id_map)} champs OK.")
            except Exception as e:
                print(f"Err map champ: {e}")
        else:
            print("Err: Format champion.json.")
    ...
    def _create_widgets(self):
        ...
        self.stats_title_label = QLabel("Stats (10 Classées)")
        ...
        self.add_button.setFixedSize(QSize(40, 40))
        self.add_button.setObjectName("ManagementButton")
        self.add_button.setToolTip("Ajouter un compte")

        mod_icon = self.style().standardIcon(QStyle.SP_FileDialogDetailedView)
        self.mod_button = QPushButton()
        self.mod_button.setIcon(mod_icon)
        self.mod_button.setIconSize(QSize(20,20))
        self.mod_button.setFixedSize(QSize(40, 40))
        self.mod_button.setObjectName("ManagementButton")
        self.mod_button.setToolTip("Modifier le compte sélectionné")


        del_icon = self.style().standardIcon(QStyle.SP_TrashIcon)
        self.del_button = QPushButton()
        self.del_button.setIcon(del_icon)
        self.del_button.setIconSize(QSize(20,20))
        self.del_button.setFixedSize(QSize(40, 40))
        self.del_button.setObjectName("ManagementButton")
        self.del_button.setToolTip("Supprimer le compte sélectionné")


        quit_icon = self.style().standardIcon(QStyle.SP_DialogCloseButton)
        self.quit_button = QPushButton()
        self.quit_button.setIcon(quit_icon)
        self.quit_button.setIconSize(QSize(20,20))
        self.quit_button.setFixedSize(QSize(40, 40))
        self.quit_button.setObjectName("ManagementButton")
        self.quit_button.setToolTip("Quitter l'application")

        self.add_button.clicked.connect(self.ajouter_compte)
        self.mod_button.clicked.connect(self.modifier_compte)
        self.del_button.clicked.connect(self.supprimer_compte)
        self.quit_button.clicked.connect(self.close)

        button_layout.addWidget(self.add_button)
        button_layout.addWidget(self.mod_button)
        ...
        launch_frame = QFrame()
        launch_frame.setObjectName("LaunchFrame")
        launch_layout = QHBoxLayout(launch_frame)
        self.launch_button = QPushButton("Lancer Compte Sélectionné")
        self.launch_button.setObjectName("LaunchButton")
        self.launch_button.setIconSize(QSize(24,24))
        self.launch_button.clicked.connect(self.lancer_connexion_auto_selected)
    ...
    def _apply_stylesheet(self):
        ...
        light_bg = "#1A2430"
        text_color = "#E0E0E0"
        text_color_muted = "#A0A0A0"
        accent1 = "#0AC8B9"
        accent1_light = "#3EF0DE"
        accent1_dark = "#08A090"
        accent2 = "#C8AA6E"
        accent2_light = "#E0C080"
        accent2_dark = "#B0905A"
        border_color = "#2A3A5A"
        selection_bg = "#2A3A5A"
        error_color = "#E84057"
        ...
        style = f"""
            LolSwitcherApp, QWidget {{
                background-color: {dark_bg};
                color: {text_color};
                font-family: Segoe UI, Helvetica, Arial;
                font-size: 10pt;
            }}
            QFrame#DetailsFrame, QFrame#LaunchFrame, QWidget#AccountWidget, QDialog {{
                background-color: {medium_bg};
                border: 1px solid {border_color};
                border-radius: 6px;
            }}
             QWidget#StatsWidget, QWidget#MasteryWidget {{
            ...
            QLabel#StatusLabel {{
             ...
             QLabel#StatusLabel[status="ready"] {{
                color: {accent1};
            }}
            ...
            QLabel#DetailsTitleLabel {{
             ...
             QLabel#AccountRankLabel {{
                font-size: 9pt;
                color: {text_color_muted};
                background-color: transparent;
            }}
             QLabel#AccountRankLabel[rankType="bold"] {{
                color: {text_color};
                font-weight: bold;
             }}
             QLabel#AccountLevelLabel {{
                font-size: 9pt;
                color: {text_color_muted};
                background-color: transparent;
                margin-top: 2px;
             }}
             QLabel#MasteryNameLevelLabel {{
                 font-size: 10pt;
                 font-weight: bold;
                 color: {text_color};
                 background-color: transparent;
             }}
             QLabel#MasteryPointsLabel {{
                 font-size: 9pt;
                 color: {text_color_muted};
                 background-color: transparent;
             }}
             QLabel#MasteryIconLabel {{
                 background-color: transparent;
             }}
            QPushButton {{
            ...
            QPushButton:hover {{
                background-color: {accent1_light};
            }}
            QPushButton:pressed {{
                background-color: {accent1_dark};
            }}
            ...
            QPushButton#LaunchButton:hover {{
                background-color: {accent2_light};
            }}
            QPushButton#LaunchButton:pressed {{
                background-color: {accent2_dark};
            }}
             QPushButton#RefreshButton, QPushButton#ManagementButton {{
              ...
              QPushButton#RefreshButton:hover, QPushButton#ManagementButton:hover {{
                 background-color: {selection_bg};
                 border: 1px solid {accent1};
              }}
              QPushButton#RefreshButton:pressed, QPushButton#ManagementButton:pressed {{
                 background-color: {dark_bg};
              }}
            ...
            QProgressBar::chunk {{
                background-color: {accent1};
                border-radius: 5px;
                /* Use width style for determinate */
            }}
            QLineEdit {{
                background-color: {light_bg};
                color: {text_color};
            ...
            QMessageBox, QDialog {{
             ...
             QDialog QPushButton:hover {{ background-color: {accent1_light}; }}
             QDialog QPushButton:pressed {{ background-color: {accent1_dark}; }}

            QWidget#AccountWidget {{
                 padding: 12px;
                 border-radius: 6px;
                 border: 1px solid {medium_bg};
            }}
            QWidget#AccountWidget:hover {{
                 background-color: {light_bg};
                 border: 1px solid {border_color};
            }}
    ...
    def trouver_image(self, image_name_in_subfolder, confidence=0.7, region=None):
        ...
        try:
            ...
            print(f"Err img load {image_path}: {e}")
            return None
        try:
            ...
            print(f"Err find_image {image_name_in_subfolder}: {e}")
        return None

    def attendre_image(self, image_name, timeout=20, confidence=0.7, region=None):
        start = time.time()
        region_txt = f"dans {region}" if region else ""
        print(f"Attente {image_name} {region_txt} (max {timeout}s)...")

        while time.time() - start < timeout:
            if hasattr(self, 'app_running') and not self.app_running: return None
            coords = self.trouver_image(image_name, confidence=confidence, region=region)
            if coords:
                print(f"{image_name} trouvé @ {coords}.")
                return coords
            time.sleep(0.5)
        print(f"Timeout: {image_name} non trouvé.")
        return None
    ...
    def cliquer_coord(self, coords, image_name_debug="coord"):
        if coords:
            ...
            print(f"Clic {image_name_debug} @({cx},{cy})")
            time.sleep(0.5)
            return True
        else:
            print(f"Err coords: {image_name_debug}")
            return False
    ...
    def _refresh_ui_list_sync(self):
        print("Rafraîchissement synchrone de l'UI...")
        self.update_status("Mise à jour liste...", "orange", False) # No progress bar here
        ...
        while self.accounts_layout.count():
             ...
             if widget:
                 widget.deleteLater()
        self.account_widgets = {}
        # Don't clear detailed_stats_cache, only list widgets

        current_time = time.time()
        sorted_keys = sorted(self.comptes.keys())
        first_key = None
        ...
        for key in sorted_keys:
            ...
            # Try to get summoner info from API cache first
            s_info = None
            cached_entry = self.api_cache.get(key, {}).get("summoner_info")
            if cached_entry and (current_time - cached_entry.get("timestamp", 0)) < CACHE_TTL_LIST:
                 s_info = cached_entry['data']
                 # Ensure this cached data is also in detailed_stats_cache for consistency
                 if key not in self.detailed_stats_cache: self.detailed_stats_cache[key] = {}
                 self.detailed_stats_cache[key]['summoner'] = s_info
            else:
                 # If not cached or stale, display basic info only for sync refresh
                 # Or potentially add the item to a list for background fetch later?
                 # For now, just display basic info derived from self.comptes
                 # We still need a dict structure similar to what get_summoner_info_sync provides
                 s_info = {
                    'display': "(Données API non chargées/obsolètes)",
                    'error': True, # Mark as error/incomplete
                    'level': 'N/A',
                    'tier': '?',
                    'rank_div': '',
                    'lp': 0,
                    'ranked_stats': None,
                    'puuid': None # PUUID might be needed for detail fetch trigger
                 }
                 # Ensure entry exists in detailed_stats_cache
                 if key not in self.detailed_stats_cache: self.detailed_stats_cache[key] = {}
                 self.detailed_stats_cache[key]['summoner'] = s_info
            ...
            r_div = s_info.get('rank_div','')
            lp = s_info.get('lp',0)
            r_stats = s_info.get('ranked_stats')
            ...
            # Only attempt load if not '?' and not 'UNRANKED'
                ...
                if os.path.exists(icon_full_path):
                    try:
                        ...
                        self.rank_icon_cache[icon_full_path] = pixmap
                         ...
                         print(f"Err icône Qt {icon_full_path}: {e}")
                         pixmap = None
                else:
                    pixmap = None
                    print(f"Icon absente: {icon_full_path}")
        ...
        # Reselect appropriate account (e.g., the one just added/modified, or first)
        # For simplicity, just select the first one for now in sync refresh
        new_sel = self.selected_account_key if self.selected_account_key in self.account_widgets else first_key
        if new_sel:
    ...
    def handle_api_results(self, result):
         ...
         if msg_type == 'list_refresh':
             self._update_ui_list_refresh(result['data'], result['previous_selection'])
    ...
    def get_region_endpoints(self, r_id):
        ...
        if not r_data:
            print(f"Warn: Région {r_id_up} inconnue->EUW.")
            r_data = self.region_map["EUW1"]
            r_id_up = "EUW1"
        return {"account": self.account_url_base.format(region_routing=r_data["routing"]),
                "summoner": self.summoner_url_base.format(region_platform=r_data["platform"]),
                "match": self.match_url_base.format(region_routing=r_data["routing"]),
                "mastery": self.mastery_url_base.format(region_platform=r_data["platform"]),}
    ...
    def get_summoner_info_sync(self, game_name, tag_line, region_platform_id):
        ...
        try:
            cl_tag=tag_line.strip().lstrip('#'); assert cl_tag, "Tag Line vide"
            enc_gn=urllib.parse.quote(game_name); enc_tl=urllib.parse.quote(cl_tag)
            ...
            if acc_r.status_code==404:
                print(f"API 404 (Account): ID Riot {game_name}#{cl_tag} introuvable.")
                return {'display':f"ID Riot introuvable.",'error':True}
            acc_r.raise_for_status();
            acc_data=acc_r.json(); puuid=acc_data.get("puuid")

            if puuid: res['puuid'] = puuid
            else:
                print(f"PUUID non trouvé dans la réponse pour {game_name}#{cl_tag}");
            ...
            summ_data=summ_r.json(); summ_id=summ_data.get("id"); level=summ_data.get('summonerLevel','N/A')
            res['level']=level;

            if not summ_id:
                print(f"ID Summoner non trouvé pour PUUID {puuid[:8]}...")
            ...
            lg_r.raise_for_status(); lg_info=lg_r.json()
            for entry in lg_info:
                if entry.get('queueType')=="RANKED_SOLO_5x5":
                    tier=entry.get('tier','UNRANKED'); rank_div=entry.get('rank',''); lp=entry.get('leaguePoints',0)
                    r_wins=entry.get('wins',0); r_losses=entry.get('losses',0); r_tot=r_wins+r_losses
                    r_wr=(r_wins/r_tot*100) if r_tot>0 else 0.0; break
            res['tier']=tier; res['rank_div']=rank_div; res['lp']=lp
            res['ranked_stats']={'wins':r_wins,'losses':r_losses,'total':r_tot,'wr':r_wr}
            res['stats']=None

            return res

        except AssertionError as e: print(f"Err assert {game_name}#{tag_line}: {e}"); res['display']=f"Erreur: {e}"; res['error']=True; return res
        except requests.exceptions.HTTPError as e:
            sc=e.response.status_code if e.response else 'N/A'; url=e.request.url if e.request else 'N/A'; api_err=f"API {sc}"
            if sc==429: api_err="Limite API atteinte"
            elif sc==404: api_err="Non trouvé (Summ/League?)"
            elif sc==403: api_err="Clé API invalide/expirée (403)"; print(f"ERREUR 403 Forbidden sur {url}. Vérifiez la clé API.")
            elif sc==401: api_err="Non autorisé (401)"; print(f"ERREUR 401 Unauthorized sur {url}\n Headers:{self.headers}")
            else: api_err = f"API {sc}"
            print(f"Err HTTP API pour {game_name}#{cl_tag}: {e}"); res['display']=f"Err API: {api_err}"; res['error']=True; return {**res, **{'display': f"Err API: {api_err}"}}
        except requests.exceptions.RequestException as e:
            print(f"Err Réseau API pour {game_name}#{cl_tag}: {e}"); res['display']="Erreur Réseau"; res['error']=True; return {**res, **{'display': "Erreur Réseau"}}
        except Exception as e:
            print(f"Err Sync Inattendu pour {game_name}#{cl_tag}: {e}"); res['display']=f"Erreur: {str(e)}"; res['error']=True; return {**res, **{'display': f"Erreur: {str(e)}"}}
    ...
    def get_champion_mastery_sync(self, puuid, region_id):
        ...
        try:
            ...
            r.raise_for_status(); data = r.json(); top = []
            ...
            if e.response.status_code != 404: print(f"Err API Maîtrise HTTP {e.response.status_code}: {e}")
        except Exception as e: print(f"Err Maîtrise PUUID {puuid[:8]}: {e}")
        return None

    def analyze_match_history_sync(self, puuid, region_endpoints):
        match_ids = self.get_match_history_sync(puuid, region_endpoints, count=10, queue_id=420)
        wins, losses = 0, 0; recent_champions = []
        default_stats = {'total_games':0, 'wins':0, 'losses':0, 'win_rate':0, 'top_champions':[]}

        if not match_ids:
            print("Aucun match classé récent trouvé.")
            return default_stats

        analyzed_count = 0

        # Potential concurrency improvement spot:
        match_details_results = []
        with self.api_executor as executor:
             future_to_match_id = {executor.submit(self.get_match_details_sync, match_id, region_endpoints): match_id for match_id in match_ids}
             for future in as_completed(future_to_match_id):
                 ...
                 try:
                     ...
                     print(f'Match detail fetch generated an exception for {match_id}: {exc}')

        for match_details in match_details_results:
            if not self.app_running: return default_stats # Check again
            if not match_details or 'info' not in match_details: continue
            ...
            for p in match_details['info'].get('participants',[]):
                if p.get('puuid') == puuid:
                    ...
                    break
            # if not participant_found: print(f"Avert: PUUID {puuid} non trouvé dans match {match_id}") # match_id not available here easily
    ...
    def get_match_history_sync(self, puuid, region_endpoints, count=20, queue_id=None):
        try:
            ...
            print(f"Fetching match history: {url}")
            r = self.http_session.get(url, timeout=10);
            r.raise_for_status()
            return r.json()
        except requests.exceptions.HTTPError as e:
             if e.response.status_code == 404:
                 queue_info = f" (queue: {queue_id})" if queue_id else ""
                 print(f"Match history 404 pour PUUID {puuid[:8]}{queue_info} (pas de parties récentes dans cette file?)")
             else:
                 print(f"Err HTTP hist. parties {puuid[:8]}: {e}")
        except Exception as e:
            print(f"Err hist. parties {puuid[:8]}: {e}")
        return []
    ...
    def get_match_details_sync(self, match_id, region_endpoints):
        try:
             ...
             print(f"Err HTTP détails match {match_id}: {e}")
        except Exception as e: print(f"Err détails match {match_id}: {e}")
        return None
    ...
    def _update_ui_list_refresh(self, fetched_data, previous_selection):
        ...
        if not fetched_data:
            ...
            error_label = QLabel("Erreur chargement liste.")
            self.accounts_layout.addWidget(error_label)
            return

        sorted_keys = sorted(fetched_data.keys())
        first_key = None
        for key in sorted_keys:
            if not first_key: first_key = key
            data = fetched_data[key]
            s_info = data.get('summoner')
            if key not in self.comptes or not s_info: continue
            ...
            icon_label = QLabel()
            icon_label.setFixedSize(24, 24)
            icon_file = f"Season_2023_-_{tier.lower()}.png"
            icon_full_path = resource_path(os.path.join("ranked_emblems", icon_file))
            pixmap = self.rank_icon_cache.get(icon_full_path)
            if not pixmap:
                if tier != 'UNRANKED' and os.path.exists(icon_full_path):
                    try:
                        pixmap = QPixmap(icon_full_path).scaled(24, 24, Qt.KeepAspectRatio, Qt.SmoothTransformation)
                        self.rank_icon_cache[icon_full_path] = pixmap
                    except Exception as e:
                    ...
                    pixmap = None
                    if tier != 'UNRANKED': print(f"Icon absente: {icon_full_path}")
            if pixmap:
                icon_label.setPixmap(pixmap)
            else:
                 icon_label.setText("")
            ...
            r_txt = f"{tier.capitalize()} {r_div}" if tier != 'UNRANKED' else "Unranked"
            lp_txt = f" {lp} LP" if tier != 'UNRANKED' else ""
            rank_label = QLabel(r_txt)
            rank_label.setObjectName("AccountRankLabel")
            rank_label.setProperty("rankType", "bold")
            lp_label = QLabel(lp_txt)
    ...
    def _update_ui_details(self, account_key, detail_data):
        if not self.app_running or account_key not in self.detailed_stats_cache:
            print(f"Skipping detail update for {account_key} (app closed or key not in cache)")
            return
        ...
        if detail_data:
            ...
            if 'mastery' in detail_data:
             ...
             print(f"No detail data received for {account_key} in _update_ui_details")


        if account_key == self.selected_account_key:
            print(f"Updating details display for selected: {account_key}")
    ...
    def _select_account_ui(self, selected_key):
        if not self.app_running or selected_key == self.selected_account_key:
             if selected_key == self.selected_account_key:
                 self.trigger_detail_fetch_if_needed(selected_key)
             return

        print(f"Account selected: {selected_key}")
        ...
        if self.selected_account_key in self.account_widgets:
            ...
            print(f"Clé widget {selected_key} non trouvée lors de la sélection.")
            self.selected_account_key = None
    ...
    def trigger_detail_fetch_if_needed(self, key_to_check):
        ...
        if key_to_check in self.detailed_stats_cache:
             ...
             if cached_mastery and (current_time - cached_mastery.get("timestamp", 0)) < CACHE_TTL_DETAIL:
                 needs_mastery_fetch = False
                 if 'mastery' not in self.detailed_stats_cache[key_to_check] or self.detailed_stats_cache[key_to_check]['mastery'] != cached_mastery['data']:
                      self.detailed_stats_cache[key_to_check]['mastery'] = cached_mastery['data']
             ...
             if cached_stats and (current_time - cached_stats.get("timestamp", 0)) < CACHE_TTL_DETAIL:
                 needs_stats_fetch = False
                 if 'stats' not in self.detailed_stats_cache[key_to_check] or self.detailed_stats_cache[key_to_check]['stats'] != cached_stats['data']:
                      self.detailed_stats_cache[key_to_check]['stats'] = cached_stats['data']
                      if key_to_check == self.selected_account_key:
                          self.afficher_stats_details(key_to_check)
        ...
        if (needs_mastery_fetch or needs_stats_fetch) and summoner_ok and puuid:
            if not self.detail_fetch_thread or not self.detail_fetch_thread.isRunning():
                print(f"Lancement thread fetch détails pour {key_to_check} (Mastery: {needs_mastery_fetch}, Stats: {needs_stats_fetch})")
    ...
    def afficher_stats_details(self, account_key):
        print(f"Affichage Stats/Maîtrise pour {account_key}")
        ...
        print(f" - Cache: summoner_error={summoner_error}, stats_loaded={stats is not None}, mastery_loaded={mastery is not None}")

        if stats is None:
            if summoner_error: self.stats_label.setText(f"Stats indisponibles.\n({summoner_display_error})")
            else: self.stats_label.setText("Chargement stats...")
            self.stats_title_label.setText("Stats (10 Classées)")
        elif stats and stats['total_games'] > 0:
            self.stats_title_label.setText(f"Stats ({stats['total_games']} Classées)")
            ...
            else: stats_text += " - Aucune donnée champ."
            self.stats_label.setText(stats_text)
        elif stats:
            self.stats_title_label.setText("Stats (0 Classées)")
            self.stats_label.setText("0 partie classée récente trouvée.")
        else:
        ...
        if mastery is None:
            if summoner_error: self.mastery_label.setText(f"Maîtrise indisponible.\n({summoner_display_error})")
            else: self.mastery_label.setText("Chargement maîtrise...")
        elif mastery:
            mastery_text = ""
            if mastery:
                 for champ in mastery:
                      mastery_text += f" - {champ['name']} (N.{champ['level']})\n   {champ['points']:,} pts\n".replace(',',' ')
            else: mastery_text = "Aucune donnée maîtrise."
    ...
    def ajouter_compte(self):
        dialog = AccountDialog(self.region_map.keys(), parent=self)
        if dialog.exec():
            new_data = dialog.get_data()
            if new_data:
                n, gn, tg, un, pw, rg = new_data
                if not all([n, gn, tg, un, pw, rg]):
                    QMessageBox.critical(self, "Erreur", "Tous les champs sont requis.")
                    return
                if not rg.upper() in self.region_map:
                    QMessageBox.critical(self, "Erreur", f"Région '{rg}' invalide.")
                    return
    ...
    def modifier_compte(self):
        ...
        if not key:
            QMessageBox.warning(self, "Erreur", "Sélectionnez un compte à modifier.")
            return
        if key not in self.comptes:
            QMessageBox.critical(self, "Erreur", f"Compte '{key}' introuvable (interne?).")
            return
        ...
        try:
            ...
            print(f"Avert: Imp déchif MDP pour modif {key}: {e}")
            d_pw = ""

        dialog = AccountDialog(self.region_map.keys(), parent=self, current_data={
            "name": key,
            "game_name": current_data.get('game_name',''),
            "tag_line": current_data.get('tag_line',''),
            "username": current_data.get('username',''),
            "password": d_pw,
            "region": current_data.get('region','EUW1')
        })
        ...
        if dialog.exec():
            ...
            if new_data:
                ...
                if not all([gn, tg, un, pw, rg]):
                    QMessageBox.critical(self, "Erreur", "Tous les champs (sauf Nom) sont requis.")
                    return
                if not rg.upper() in self.region_map:
                    QMessageBox.critical(self, "Erreur", f"Région '{rg}' invalide.")
                    return
                try:
                    ...
                    QMessageBox.critical(self, "Erreur Chiffrement", f"Erreur chiffrement mot de passe:\n{e}")
                    return
                self.comptes[key] = {"game_name":gn, "tag_line":tg.lstrip('#'), "username":un, "password":pwc, "region":rg.upper()}
                sauvegarder_comptes(self.comptes)
    ...
    def supprimer_compte(self):
        key = self.selected_account_key
        if not key:
            QMessageBox.warning(self, "Erreur", "Sélectionnez un compte à supprimer.")
            return
        if key not in self.comptes:
             QMessageBox.critical(self, "Erreur", f"Compte '{key}' introuvable (interne?).")
             return
        ...
        reply = QMessageBox.question(self, "Confirmation",
                                       f"Êtes-vous sûr de vouloir supprimer le compte '{key}' ?\nCette action est irréversible.",
    ...
    def lancer_connexion_auto(self, nom_key):
        print(f"Lancement auto pour: {nom_key}")
        if nom_key not in self.comptes:
            self.update_status("Err: Compte introuvable.", "red", False)
            QMessageBox.critical(self, "Erreur Interne", f"Compte '{nom_key}' non trouvé dans les données.")
            return
        try:
            ...
            if not user or not pwd_c:
                ...
                QMessageBox.critical(self, "Erreur Données", f"Login ou mot de passe manquant pour '{nom_key}'. Veuillez modifier le compte.")
                return
            pwd = cipher.decrypt(pwd_c.encode()).decode()
        except Exception as e:
            self.update_status("Err déchiffrement MDP.", "red", False)
            QMessageBox.critical(self, "Erreur Données", f"Erreur lors du déchiffrement du mot de passe pour '{nom_key}':\n{e}")
            return
    ...
    def fermer_client_lol_sync(self):
        self.launch_thread.update_status_signal.emit("Fermeture clients LoL/Riot...", "orange", False); found_terminated=False
        ...
        print(f"Tentative de terminaison des processus: {', '.join(p_names)}")

        for p in psutil.process_iter(['pid','name']):
            try:
                if p.info['name'] in p_names:
                     print(f"Terminaison: {p.info['name']} (PID: {p.info['pid']})")
                     p.terminate(); found_terminated=True
            except (psutil.NoSuchProcess, psutil.AccessDenied): continue
            except Exception as e: print(f"Erreur terminaison {p.info.get('name','?')}: {e}")

        if found_terminated: time.sleep(3.5)

        killed = False
        for p in psutil.process_iter(['pid','name']):
            try:
                if p.info['name'] in p_names:
                    print(f"Kill forcé: {p.info['name']} (PID: {p.info['pid']})")
                    p.kill(); killed=True
            except (psutil.NoSuchProcess, psutil.AccessDenied): continue
            except Exception as e: print(f"Erreur kill {p.info.get('name','?')}: {e}")

        if killed: time.sleep(1.5)

        still_running = [p.info['name'] for p in psutil.process_iter(['name']) if p.info['name'] in p_names]
        if still_running:
             print(f"Avert: Processus encore actifs: {', '.join(still_running)}")
             self.launch_thread.update_status_signal.emit("Fermeture partielle.", "orange", False); return False
        else:
             self.launch_thread.update_status_signal.emit("Clients fermés.", "green", False); return True


    def lancer_client_lol_sync(self, username, password):
        riot_paths = ["C:\\Riot Games\\Riot Client\\RiotClientServices.exe",
                      os.path.expanduser("~\\AppData\\Local\\Riot Games\\Riot Client\\RiotClientServices.exe")]
        ...
        if not rp:
             ...
             self.launch_thread.show_message_signal.emit("Erreur Chemin",f"Impossible de trouver RiotClientServices.exe dans les chemins:\n{chr(10).join(riot_paths)}\nVérifiez l'installation de Riot Games.")
             return False

        self.launch_thread.update_status_signal.emit("Lancement Riot Client...","orange",False)
        try:
            subprocess.Popen([rp]); print(f"Client Riot lancé via: {rp}")
        except Exception as e:
        ...
        if not attendre_processus("Riot Client.exe", timeout=30):
            self.launch_thread.update_status_signal.emit("Err: Processus 'Riot Client.exe' non détecté.", "red", False)
            self.launch_thread.show_message_signal.emit("Timeout Processus", "Le processus 'Riot Client.exe' n'a pas démarré à temps.")
            return False

        self.launch_thread.update_status_signal.emit("Activation fenêtre Riot...", "orange", False)
        hwnd_riot = None; start_time = time.time()
        timeout_activation = 20
        while time.time() - start_time < timeout_activation:
            hwnd_riot = activer_fenetre_riot()
            if hwnd_riot:
                 if win32gui.GetForegroundWindow() == hwnd_riot:
                     break
            time.sleep(0.75)
        else:
            print(f"Timeout ({timeout_activation}s) ou échec activation fenêtre Riot.")
            if not hwnd_riot:
                 self.launch_thread.update_status_signal.emit("Err: Fenêtre Riot introuvable.", "red", False)
                 self.launch_thread.show_message_signal.emit("Erreur Fenêtre", "Impossible de trouver la fenêtre Riot Client.")
                 return False
            else:
                 print("Avert: Fenêtre Riot trouvée mais non activée. Tentative d'automatisation quand même.")
        ...
        print("Recherche du champ Username...")
        ...
        try:
            ...
            print("Username collé.")
            time.sleep(0.3)
        except Exception as e:
            print(f"Erreur collage username: {e}")
            self.launch_thread.show_message_signal.emit("Erreur Automatisation", f"Erreur collage username:\n{e}")
            return False

        try:
            print("Appui sur Tab pour passer au mot de passe...")
            pyautogui.press('tab')
            time.sleep(0.3)
            print("Collage du mot de passe...")
            pyperclip.copy(password)
            pyautogui.hotkey('ctrl', 'v')
            print("Password collé.")
            time.sleep(0.5)
        except Exception as e:
            print(f"Erreur Tab/Collage password: {e}")
            self.launch_thread.show_message_signal.emit("Erreur Automatisation", f"Erreur Tab/Collage password:\n{e}")
            return False

        print("Recherche du bouton Valider/Login...")
        ...
        if not login_button_coords:
            print("'VALIDER IDENTIFIANTS.png' non trouvé, tentative 'LOGIN_BUTTON_ALT.png'...")
        ...
        if play_coords:
            ...
            play_coords = self.attendre_image_sync("Play.png", timeout=10, confidence=0.7, region=play_region)
             ...
             print("Avert: 'beforeplay.png' non trouvé, recherche directe de 'Play.png'.")
             play_coords = self.attendre_image_sync("Play.png", timeout=15, confidence=0.7, region=play_region)

        if play_coords:
             print("'Play.png' trouvé, clic...")
             ...
             print("Avert: Bouton 'Play' final non trouvé. Connexion peut avoir réussi mais client pas lancé?")
...
class AccountDialog(QDialog):
    def __init__(self, region_list, parent=None, current_data=None):
        ...
        for i, label_text in enumerate(field_labels):
            ...
            if label_text == "Nom:" and self.is_edit_mode:
                ...
                layout.addWidget(entry, i, 1, 1, 2)
        ...
        if self.is_edit_mode:
            ...
            self.fields["Région:"].setText(current_data.get('region', 'EUW1'))
        elif not self.is_edit_mode:
             self.fields["Région:"].setText("EUW1")
    ...
    def get_data(self):
        ...
        return (
            ...
            self.fields["Région:"].text().strip().upper()
        )


if __name__ == "__main__":
    os.makedirs(CACHE_DIR, exist_ok=True)
    os.makedirs(resource_path("CV_Images"), exist_ok=True)
    os.makedirs(resource_path("ranked_emblems"), exist_ok=True)
